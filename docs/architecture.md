...
- The first dataset export path now exists:
  - `POST /api/v1/datasets/jobs` supports `export_kind = "bars"`, `export_kind = "ticks"`, and `export_kind = "backtest_trades"`
  - the worker reads persisted bars or ticks from ClickHouse, or persisted run trades from Postgres, writes Parquet plus a manifest under `artifacts/`, and records the export in Postgres
